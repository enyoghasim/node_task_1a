...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new usersAdminAddViewModel(
      db.user,
      "Add user",
      "",
      "",
...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new usersAdminEditViewModel(
      db.user,
      "Edit user",
      "",
      "",
...
app.post(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      let data = await db.user.edit(
        { first_name, last_name, role_id, image, phone, status },
        id
      );
      if (!data) {
        viewModel.error = "Something went wrong";
      ...
      if (resourceExists.credential) {
        data = await db.credential.edit(
          helpers.filterEmptyFields({
            email: credential_email,
            password: credential_password,
          }),
          resourceExists.credential.id
        );
        if (!data) {
          viewModel.error = "Something went wrong";
          return res.render("admin/Edit_Users", viewModel);
...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      var viewModel = new usersAdminDetailViewModel(
        db.user,
        "User details",
        "",
        "",
      ...
      data["credential.two_factor_authentication"] =
        db.credential.two_factor_authentication_mapping()[
          data.credential.two_factor_authentication
        ];
      data["credential.type"] =
        db.credential.type_mapping()[data.credential.type];
      data["credential.status"] =
        db.credential.status_mapping()[data.credential.status];
      data["credential.verify"] =
